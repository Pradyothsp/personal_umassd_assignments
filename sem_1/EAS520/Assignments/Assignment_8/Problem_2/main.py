...
for i in range(2000):
    ...
    if rank == 0:
        ...
        if is_round_1:
            bet_amount = get_bet_amount(current_amount=P2_AMOUNT, is_round_1=is_round_1)

        else:
            bet_amount = get_bet_amount(P2_AMOUNT, P2_WIN_STATUS[-1], P2_BETS[-1], is_round_1)

        P2_AMOUNT -= bet_amount
        P2_BETS.append(bet_amount)

        data_send = {
            'bet_amount': bet_amount,
            'coin_choice': random.choices(['H', 'T'])
        }
        comm.send(data_send, dest=0, tag=120)

        comm.barrier()
        data_rec = comm.recv(source=0, tag=102)

        P2_WIN_STATUS.append(data_rec.get('win_status'))
        P2_AMOUNT += data_rec.get('amount_won')

        print(f"P2_AMOUNT: {P2_AMOUNT}, after {i + 1} iterations")

    elif rank == 3:
        if is_round_1:
            bet_amount = get_bet_amount(current_amount=P3_AMOUNT, is_round_1=is_round_1)
        else:
            bet_amount = get_bet_amount(P3_AMOUNT, P3_WIN_STATUS[-1], P3_BETS[-1], is_round_1)

        P3_AMOUNT -= bet_amount
        P3_BETS.append(bet_amount)
        ...
        comm.send(data_send, dest=0, tag=130)

        comm.barrier()
        data_rec = comm.recv(source=0, tag=103)

        P3_WIN_STATUS.append(data_rec.get('win_status'))
        P3_AMOUNT += data_rec.get('amount_won')

        print(f"P3_AMOUNT: {P3_AMOUNT}, after {i + 1} iterations")
